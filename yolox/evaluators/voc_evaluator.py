...
class VOCEvaluator:
    ...
    def evaluate(
        ...
        if trt_file is not None:
            ...
            y = torch.ones(1, 3, test_size[0], test_size[1]).cuda()
            y2 = torch.ones(1, 3, test_size[0], test_size[1]).cuda()
            x1 = torch.ones(1, 3, test_size[0], test_size[1]).cuda()
            x2 = torch.ones(1, 3, test_size[0], test_size[1]).cuda()
            y1 = torch.ones(1, 3, test_size[0], test_size[1]).cuda()
            model(x,x1,x2,y,y1,y2)
            model = model_trt

        for cur_iter, ((imgs,imgs1,imgs2,timg,timg1,timg2,_ , info_imgs,_, ids)) in enumerate(
                 progress_bar(self.dataloader)
        ):
            with torch.no_grad():
                imgs = imgs.type(tensor_type)
                imgs1 = imgs1.type(tensor_type)
                imgs2 = imgs2.type(tensor_type)
                timg = timg.type(tensor_type)
                timg1 = timg1.type(tensor_type)
                timg2 = timg2.type(tensor_type)
                ...
                outputs = model(imgs,imgs1,imgs2,timg,timg1,timg2)
